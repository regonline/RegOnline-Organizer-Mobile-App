/***********Attendee List************/
rol.AgendaAttendeeList = Ext.extend(Ext.Panel, {
    title: 'Agenda Attendees',
    id: '_agendaAttendeeListPanel',
    layout: 'fit',
    EventID: null,
    cfid: null,
    CallBackFn: null,
    ...
    initComponent: function() {
        ...
        this.list = new Ext.List({
            ...
            itemTpl: [
                                    '<tpl for=".">',
                                        '<div class="attendeeItem">',
                                             '<div class="attendeeData {Registration.StatusDescription}">',
                                                          '<ul class="aTarget" id="{Registration.ID}">',
                                                            '<h4>{Registration.LastName}, {Registration.FirstName}</h4>',
                                                            '<li>#{ID}</li>',
                                                            '<li><a href="mailto:{Email}">{Registration.Email}</a></li>',
                                                            '<li>{Registration.Company}</li>',
                                                            '<li>{Registration.City} {Registration.State} {Registration.Country}</li>',
                                                          '</ul>',
                                              '</div>',
                                               '<div class="chk {[rol.Main.ShowCheckinBox(values.Registration.StatusDescription)]}"></div>',
                                         '</div>',
                                    '</tpl>'
                                ],
            grouped: true,
            ...
            store: new Ext.data.JsonStore({
                model: 'attendeeList',
                sorters: 'Registration.LastName',
                groupDir: 'ASC',
                getGroupString: function(record) {
                    return record.get('Registration.StatusDescription');
                }
            })
        });
        this.list.on('containertap', this.onAttendeeTap, this);
        this.items = [this.list].concat(this.items || []);
        rol.AgendaAttendeeList.superclass.initComponent.call(this);
    },
    ...
    CancelSearch: function() {
        ...
        this.list.store.sort([{ property: 'Registration.StatusDescription', direction: 'DESC' }, { property: 'Registration.LastName', direction: 'ASC'}]);
    },
    ...
    ValidateCheckin: function(id) {
        if (id.length > 5) {
            var index = rol.Main.app.agendaResponseStore.find('ID', id);
            if (index < 0) {
                rol.Main.app.onError('<div class="errorText fail">Attendee is not valid</div>');
                return false;
            }
            var attendee = rol.Main.app.agendaResponseStore.getAt(index);
            //check current status to ensure not already checked in
            var status = attendee.get('StatusDescription');
            ...
            if (rol.Main.app.agendaResponseStore != null) {
                ...
                for (j = 0; j < attArr.length; j++) {
                    for (i = 0; i < rol.Main.app.agendaResponseStore.data.length; i++) {
                        ...
                        if (id == attArr[j]) {
                            rol.Main.app.agendaResponseStore.data.items[i].set('Registration.StatusDescription', 'Attended');
